...
async function loadItems() {
  try {
    ...
    if (res.ok) {
      ...
      items = JSON.parse(localStorage.getItem('app_construct_automated_qa___boundary_test_suite') || '[]');
    }
  } catch(e) {
    items = JSON.parse(localStorage.getItem('app_construct_automated_qa___boundary_test_suite') || '[]');
  }
...
function saveLocal() {
  localStorage.setItem('app_construct_automated_qa___boundary_test_suite', JSON.stringify(items));
}
